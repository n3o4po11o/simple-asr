...
$crt = Get-ChildItem "$vsRedist\*\VC\Redist\MSVC\*\x64\Microsoft.VC143.CRT" `
    -Directory -ErrorAction SilentlyContinue | Sort-Object FullName -Descending | Select-Object -First 1
$omp = Get-ChildItem "$vsRedist\*\VC\Redist\MSVC\*\x64\Microsoft.VC143.OpenMP" `
    -Directory -ErrorAction SilentlyContinue | Sort-Object FullName -Descending | Select-Object -First 1
if (-not $crt -or -not $omp) { throw "VC Redist 目录未找到（构建机需 VS C++ 工作负载）" }
