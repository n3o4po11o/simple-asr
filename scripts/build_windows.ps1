...
# （v0.1.7 真机首验实测）。从构建机 VS 的 Redist 目录取（任一版本目录）。
$vsRedist = "C:\Program Files\Microsoft Visual Studio\2022"
# Get-Item 取匹配目录本身（Get-ChildItem -Directory 列的是【子】目录——
# Microsoft.VC143.CRT 里只有 dll 文件，永远空结果，v0.1.8 实测翻车）
$crt = Get-Item "$vsRedist\*\VC\Redist\MSVC\*\x64\Microsoft.VC143.CRT" `
    -ErrorAction SilentlyContinue | Sort-Object FullName -Descending | Select-Object -First 1
$omp = Get-Item "$vsRedist\*\VC\Redist\MSVC\*\x64\Microsoft.VC143.OpenMP" `
    -ErrorAction SilentlyContinue | Sort-Object FullName -Descending | Select-Object -First 1
if (-not $crt -or -not $omp) {
    # 未命中时把 Redist 树打出来——一轮日志定位布局差异，不再盲猜
    Get-ChildItem "$vsRedist\*\VC\Redist" -Recurse -Directory -ErrorAction SilentlyContinue |
        Select-Object -First 10 | ForEach-Object { Write-Host "redist-dir: $($_.FullName)" }
    throw "VC Redist 目录未找到（构建机需 VS C++ 工作负载含 Redist 组件）"
}
Copy-Item (Join-Path $crt.FullName "*") $Out -Force
Copy-Item (Join-Path $omp.FullName "*") $Out -Force
Info "VC++ 运行库已随包（$($crt.Parent.Parent.Name)）"
