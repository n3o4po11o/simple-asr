...
fn acpp_transcribe_thread(
    ...
    let result = (|| -> Result<(String, String), String> {
        ...
        // **计算并行**——与流式转写同时跑（分段走 GPU、声纹走 CPU，互不抢核），
        // 文本即刻开始上屏，不再有「转写前 CPU 拉满干等」的阶段；说话人归属
        // 在收尾统一回填（词级对齐或块级重叠）。
        //
        // 顺序硬约束：**先建完流式会话（start_stream）再 spawn diar 线程**。
        // 引擎的设备/后端是进程级懒初始化——两会话并发创建时，diar 线程可能
        // 拿到构造中途的后端（buffer_type 的 iface 尚未填充 → 空函数指针，
        // v0.1.0/v0.1.1 在 bazzite 上 SIGSEGV 的实锤栈：SortformerDiarSession
        // ctor → ggml_gallocr_new → buft->iface.get_alignment == null）。
        // start_stream 返回即初始化完成，此后 diar 的会话创建安全；两者
        // 的推理计算仍并行（diar 在流式 push 循环期间跑）。
        ...
        // 注意：必须在下方 spawn diar 线程**之前**完成——见上方顺序硬约束。
