...
/// 并行分离+流式探针：engine-smoke <audio> pardiar [num_speakers] [race|fix]
/// 复刻 app engine.rs 的并行结构（diar 线程与流式 ASR 同时跑）。
/// race = 先 spawn diar 再 start_stream（App 的崩溃顺序：两会话并发创建，
/// 引擎全局后端懒初始化竞态——diar 抓到构造一半的 backend，vtable 全零）；
/// fix = 先 start_stream 再 spawn（会话创建串行化，计算仍并行）。
fn pardiar_main(audio: &str) -> Result<(), Box<dyn std::error::Error>> {
    let num_speakers: u32 = std::env::args().nth(3).and_then(|s| s.parse().ok()).unwrap_or(2);
    let fixed = std::env::args().nth(4).map(|s| s == "fix").unwrap_or(false);
    let backend = if cfg!(target_os = "macos") { "metal" } else { "vulkan" };
    let pcm = asr_core::audio::load_audio_16k_mono(std::path::Path::new(audio))?;
    println!(
        "audio     : {:.1}s k={num_speakers} 顺序={}",
        pcm.len() as f64 / 16000.0,
        if fixed { "fix（先建流会话）" } else { "race（先 spawn diar）" }
    );
    let _ = num_speakers; // 探针只跑 sortformer 分段，聚类阶段不在此复现范围

    let m = audiocpp_ffi::AcppModel::load(&asr_core::model::model_dir(), "qwen3_asr")
        .map_err(|e| e.to_string())?;
    let spawn_diar = |pcm: Vec<f32>| {
        let backend = backend.to_string();
        std::thread::spawn(move || {
            let d = audiocpp_ffi::AcppModel::load(&asr_core::model::diar_model_path(), "sortformer_diar")
                .map_err(|e| e.to_string())?;
            let turns = d.diarize(&pcm, &backend, 0, 4).map_err(|e| e.to_string())?;
            Ok::<Vec<audiocpp_ffi::SpeakerSegment>, String>(turns)
        })
    };

    let t0 = std::time::Instant::now();
    let (handle, stream) = if fixed {
        let stream = m.start_stream(&backend, 0, 4, "", 5.0, pcm.len()).map_err(|e| e.to_string())?;
        let handle = spawn_diar(pcm.clone());
        (handle, stream)
    } else {
        let handle = spawn_diar(pcm.clone());
        let stream = m.start_stream(&backend, 0, 4, "", 5.0, pcm.len()).map_err(|e| e.to_string())?;
    ...
    for chunk in pcm.chunks(16000) {
        ...
        if let Some(delta) = ev.text {
            live += delta.chars().count();
        }
    }
    let (text, lang) = stream.finish().map_err(|e| e.to_string())?;
    let turns = handle.join().map_err(|_| "diar 线程 panic/崩溃".to_string())?
        .map_err(|e| e.to_string())?;
    println!(
        "并行完成  : {:.1}s，asr {} 字符（live {live}，lang {lang}），diar {} 段",
