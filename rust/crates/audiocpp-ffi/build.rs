...
fn main() {
    ...
    // 静态库扩展名与命名随工具链：GNU/Ninja 风格 lib*.a；Windows MSVC 无
    // lib 前缀（CMake 产物即 engine_runtime.lib，v0.1.5 CI 实测）
    ...
    match tree {
        BackendTree::Vulkan => {
            if cfg!(target_os = "windows") {
                ...
                // vulkan-1.lib 在 Vulkan SDK 的 Lib 目录——安装器设的
                // VULKAN_SDK 对 CI 后续步骤/开发者各自环境均可见，显式给出
                // 搜索路径（MSVC 工具链默认路径不含 SDK）。
